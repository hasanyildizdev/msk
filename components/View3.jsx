...
function View3() {
  ...
  useEffect(()=>{
    const showUp = ()=>{
      if(parseInt(window.innerWidth)>600){
        if(window.scrollY>(parseInt(window.innerHeight)*3)-120){
          setLeft('-0.3vw');
        }
      }
      else{
        if(window.scrollY>parseInt(window.innerHeight)*1.6){
          setLeft('-0.3vw');
        }
      }
  ...
  return (
    <div className={styles.container}>
        ...
        <NextImage src={require('../img/round3.png')} style={{borderColor:colors.color_text}} className={styles.roundImageDiv}  alt="" loading="lazy" objectFit="contain"/>  
        ...
        <div className={styles.textCartDiv} 
          ...
          <div className={styles.textCartItems}>
            <h1 className={styles.textCartTitle}>{t('QualityProduct')}</h1>
          </div>
          <div className={styles.textCartItems}>
            <p className={styles.textCartText} style={{fontWeight:'bold'}}>{t('View3Explanation')}</p>
          </div>
          <div className={styles.textCartItems}>
            <button 
              className={styles.textCartButton}  
              style={{borderWidth:'0.3vw',borderColor:colors.color_text,color:colors.color_text}}>
              <a href="/gallery" style={{fontWeight:'bolder'}}>{t('SeePictures')}</a>
            </button>
          </div>
        </div>
